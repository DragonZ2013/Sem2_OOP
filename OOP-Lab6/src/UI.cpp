...
void Console::user_login(){
    std::cout << "Please enter your name (case sensitive):" << std::endl;
    std::string username;
    std::getline(std::cin, username);
    if(controller.pos_by_username(username) == -1){
        controller.add_user(username);
        controller.set_current_user(username);    
    }
    else{
        controller.set_current_user(username);
    }
...
void Console::run_console() {
    bool done = false,user_logged = false;
    std::cout << "Welcome!\n";
    scan_in_file();
...
void Console::manager_update_car() {
    ...
    std::getline(std::cin, price);
    while(!Check_Double_Valid(price)&&price!="")
    {
        std::cout << "Car price is invalid\n";
    ...
    std::getline(std::cin, performance);
    while(!Check_Int_Valid(performance)&&performance!="")
    {
        std::cout << "Car performance is invalid\n";
...
void Console::add_user_favourites(){
    ...
    while(!Check_Int_Valid(id_old))
    {
        std::cout << "Car ID invalid\n";
        std::cout << "Enter Car ID:";
        std::getline(std::cin, id_old);
    }
    controller.add_favourite_current(stoi(id_old));
}

void Console::print_user_favourites() {
    for (auto &element : controller.return_favourite_current())
        std::cout << element.car_to_string() << "\n";
}
...
void Console::scan_in_file(){
    controller.read_in_cars();
}

void Console::print_out_file(){
    controller.print_out_cars();
}
